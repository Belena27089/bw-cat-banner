...
class BWCatMenu extends ObjectModel
{
    public $id_item;
    public $id_parent;
    public $id_shop;
    ...
    {
        Shop::addTableAssociation('bwcatbanner', array('type' => 'shop'));
...
//    }
    ...
    *****/
    public function updateItem()
    {
        ...
        if (!Tools::getValue('id_tab')) {
            ...
            if (!Db::getInstance()->insert('bwcatbanner', $data) || !$id_tab = Db::getInstance()->Insert_ID()) {
                return false;
            }
            ...
            foreach ($languages as $language) {
                ...
                if (!Tools::isEmpty(trim(Tools::getValue('name_'.$language['id_lang'])))) {
                    if (!Db::getInstance()->insert('bwcatbanner_lang', array(
                        'id_item' => $id_tab,
         ...
         // if item exist update it
            ...
            if (!Db::getInstance()->update('bwcatbanner', $data, '`id_item` ='.$id_item.' AND `id_shop` ='.$id_shop)
                || Db::getInstance()->delete('bwcatbanner_lang', 'id_item ='.$id_item)) {
    ...
    public function getItem($id_item = 0)
    {
        ...
        foreach ($languages as $language) {
            $sql = 'SELECT `title`, `badge`
                FROM '._DB_PREFIX_.'bwcatbanner_lang
                WHERE `id_item` = '.$id_item.'
    ...
    *****/
    public function deleteItem()
    {
        ...
        $file = Bwcatbanner::stylePath().Db::getInstance()->getValue($sql).'.css';

        if (file_exists($file)) {
            @unlink($file);
            Bwcatbanner::generateUniqueStyles(); // refresh custom css file
        }

        if (!Db::getInstance()->delete('bwcatbanner', '`id_item` ='.$id_item.' AND `id_shop` = '.$id_shop)
            || !Db::getInstance()->delete('bwcatbanner_lang', '`id_item` ='.$id_item)
            || !Db::getInstance()->delete('bwcatbanner_items', '`id_tab` ='.$id_item)) {
            return false;
        }
        return true;
    }
    ...
    *****/
    ...
    {
        if ($active) {
            $active = 'AND bw.`active` = 1';
        } else {
            $active = '';
        }
        $sql = 'SELECT bw.*, bwl.`title`, bwl.`badge`
                FROM `'._DB_PREFIX_.'bwcatbanner` bw
                LEFT JOIN `'._DB_PREFIX_.'bwcatbanner_lang` bwl
                ON (bw.`id_item` = bwl.`id_item`)
                WHERE bw.`id_shop` = '.(int)Context::getContext()->shop->id.'
                AND bwl.`id_lang` = '.(int)Context::getContext()->language->id.'
                '.$active.'
    ...
    public function addMenuItem($id_tab, $ajaxdata = false)
    {
        if (Tools::getValue('issimplemenu')) {
            ...
            if (!$this->checkItemExist($id_tab)) {
                ...
                if (!Db::getInstance()->insert('bwcatbanner_items', $data)) {
                    return false;
                }
            ...
            Db::getInstance()->delete('bwcatbanner_items', '`id_tab` = '.$id_tab.' AND `is_mega` = 1');

            $alldata = Tools::getValue('bwcatmenu_options');
            if ($ajaxdata) {
                $alldata = $ajaxdata;
            }
    ...
    ******/
    ...
    {
        $sql = 'SELECT `id`
                FROM '._DB_PREFIX_.'bwcatbanner_items
                WHERE `id_tab` = '.$id_tab.'
                AND `is_mega` = '.$is_mega;
    ...
    *****/
    public function getBwcatmenuRow($id_tab)
    {
        $rows = array();
        $sql = 'SELECT `row` 
                FROM '._DB_PREFIX_.'bwcatbanner_items
    ...
    *****/
    public function getBwcatmenuRowCols($id_tab, $row)
    {
        $sql = 'SELECT * 
                FROM '._DB_PREFIX_.'bwcatbanner_items
                WHERE `id_tab` = '.$id_tab.'
